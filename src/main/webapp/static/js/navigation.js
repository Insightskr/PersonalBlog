    ...
    loginCheck: function () {
        $("#login_name_input").blur(function () {
            ...
            if (!regaxName.test(name)) {
                ...
                show_validate_msg("#login_name_input", "success", null);
                //todo:这里应该还有一个检查用户名是否存在的函数，到时候再做。
                $("#login_btn").attr("login-able", "true");
            }
        });
    },
...
var registerFunction = {
    ...
    //检查字段是否存在
    checkExist: function (element, url) {
        $.ajax({
            url: url,
            data: "val=" + $(element).val(),
            type: "get",
...
var tool = {
    //每次显示之前，先清空先前的元素的属性
    clear_validate_msg: function (element) {
        $(element).parent().removeClass("has-success has-error");
        $(element).next("span").text("");
    },
    //显示校验信息
    show_validate_msg: function (element, status, msg) {
        tool.clear_validate_msg(element);
        if ("success" === status) {
            $(element).parent().addClass("has-success");
            $(element).next("span").text(msg);
        } else if ("error" === status) {
